...
namespace GoogleARCore.Examples.CloudAnchors
{
    ...
    public class CloudAnchorsExampleController : MonoBehaviour
    {
        ...
        private int interactableLayerMask = 1 << 8;
...
#pragma warning restore 618
        ...
        public void Update()
        {
            ...
            // Different raycast depends on isPut
            ...
            {
                RaycastHit interactHit;
                // Raycast against the interactable objects
                // ARCoreWorldOriginHelper.Raycast(touch.position.x, touch.position.y, TrackableHitFlags.PlaneWithinPolygon, out interactHit) 
                ...
                if (Physics.Raycast(near, far - near, out interactHit, 10, interactableLayerMask, QueryTriggerInteraction.Ignore))
                {
                    ...
                    {
                        interactHit.collider.gameObject.GetComponent<DoorController>().Act();
                    }
                }
            }
            ...
            {
                GameObject.Find("Door").GetComponent<DoorController>().Act();
            }
            ...
            if (m_LastHitPose != null)
            {
                ...
                else if (!IsOriginPlaced && m_CurrentMode == ApplicationMode.Hosting)
                {
                    if (Application.platform != RuntimePlatform.IPhonePlayer)
                    {
                        ...
                        elevation = arcoreHitResult.Pose.position.y;
                    }
                    else
                    {
                        m_WorldOriginAnchor = m_ARKit.CreateAnchor(m_LastHitPose.Value);
                        elevation = m_LastHitPose.Value.position.y;
                    }
                    Debug.Log(elevation);
        ...
        private void _InstantiateStar()
        {
            ...
            // Detect the Orientation of the plane
            bool isHorizontal = false;
            if (Mathf.Abs(Vector3.Dot(Vector3.Normalize(m_LastHitPose.Value.rotation * Vector3.up), Vector3.up)) > 0.5f)
                isHorizontal = true;

            // Detect the Elevation of the plane
            bool isBase = true;
            ...
            // Decide which prefab to initiate
            ...
            {
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                .CmdSpawnLadder(m_LastHitPose.Value.position, m_LastHitPose.Value.rotation);
            }
            else
            {
                GameObject.Find("LocalPlayer").GetComponent<LocalPlayerController>()
                .CmdSpawnPawn(m_LastHitPose.Value.position, m_LastHitPose.Value.rotation);
            }
...
#pragma warning restore 618
        ...
        static public void ChangeBehave()
        {
            isPut = !isPut;
        }
    }
